...
\begin{aligned}
L(\theta)&=\frac{1}{n}\sum_{i=1}^n \log p_\theta(x_i)\\
&=\mathbb{E}_{p_{data}}\big[\log p_\theta(x)\big].
\end{aligned}
$$
...
$$D_{KL}(p\parallel q)=\mathbb{E}_p\Bigg[\log\frac{p(x)}{q(x)}\Bigg].$$
...
$$
\begin{aligned}
D_{KL}(p_{data}\parallel p_\theta)&=\mathbb{E}_{p_{data}}\big[\log p_{data}(x)\big]-\mathbb{E}_{p_{data}}\big[\log p_\theta(x)\big]\\
&=-\text{Entropy}(p_{data})-L(\theta).
\end{aligned}
$$ 
